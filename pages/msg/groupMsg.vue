...
<script>
	import  {s_upImg,s_chatRecord,emUrl}  from  '@/common/js/config.js'
	import {
	    mapState,
		mapMutations
	} from 'vuex'
	export default {
		...
		onLoad(option) {
			...
			this.otherid = option.gid;
			this.ckey  = "sto_group"+this.myuid + "0" + this.otherid;
			this.getMsgList();
			
			uni.$on('msg_group', this.screenMsg);  // 接收消息
		...
		onUnload(){ // 更新main的消息列表
			...
			if(this.newMsg){
				var res = {
					...
					id: this.oinfo.gid,
					gid: this.oinfo.gid,
					groupname: this.oinfo.groupname,
					username: this.oinfo.groupname,
			...
			uni.$off('msg_group', this.screenMsg);
		},
		onNavigationBarButtonTap(e) {
			uni.navigateTo({
				url:"../msg/grouppro?id="+this.otherid+"&name="+this.oinfo.groupname
			})
		},
		methods:{
			...mapMutations(['logout']),
			...
			loadHistory(e){
				...
				uni.request({
					url: s_chatRecord,
					dataType: 'json',
					data: {token: this.s_token, type: 2, ct: this.msgList.length, id: this.oinfo.gid},
					method: 'post',
					header: {  
				...
				// 	// 消息列表
				// 	let list = [
				// 		{cid:10,msg_type:0,id:0,username:"ww",avatar:"/Static/tx.png",type:"friend",content:{text:"欢迎1233"},mine:false,fromid:1,timestamp:1574473437000},
				// 		{cid:11,msg_type:0,id:1,username:"sdf(173566)",avatar:"/Static/tx.png",type:"friend",content:{text:"欢迎123"},mine:false,fromid:1,timestamp:1574473437000},
				// 	]
				// 	// 获取消息中的图片,并处理显示尺寸
				// 	for(let i=0;i<list.length;i++){
				// 		if(list[i].msg_type==1){
				// 			list[i].content = this.setPicSize(list[i].content);
				// 			this.msgImgList.unshift(list[i].content.url);
				// 		}
				// 		list[i].cid = Math.floor(Math.random()*1000+1);
			...
			sendMsg(content,type,bemsg){
				...
				lastid++;
				console.log(lastid);
				let msg = {cid:lastid,msg_type:type,id: this.$store.state.socket.mine.id,username:this.$store.state.socket.mine.nickname,avatar:this.$store.state.socket.mine.avatar,type:"group",content:content,timestamp:timestamp,toid:this.otherid,token: this.s_token,groupname: this.oinfo.groupname}
